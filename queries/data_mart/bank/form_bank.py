...
def _parse_date(b_date: str) -> date:
    ...
    преобразование ее на начало следующего месяца для расчета коэффициентов.
...
def _parse_gap_dates(start_date: str, end_date: str) -> list:
    ...
    Преобразование в объект date,
    преобразование их на начало следующего месяца для расчета коэффициентов,
...
def _mart_bank(
    ...
    Расчитывает коэффициенты надежности и эффективности для банка, не включая
    уже расчитанных.
    ...
    ratios = get_ratios.get_bank_ratios(
        connection, meta, name, list(
            set(b_dates). difference(parsed_query)))
    return ratios
...
def form_bank_mart(
        ...
        end_date: str):
    """
    Аргументы: соединение с бд, метаданные бд, имя банка и начальная и
    конечная дата(месяцев)
    в виде строк. Расчитывает значения коэффициентов для банка за промежуток
    времени и дополняет витрину данных.
    """
    # Преобразование начальной и конечной даты(месяця) в виде строки в список
    # дат за этот промежуток
