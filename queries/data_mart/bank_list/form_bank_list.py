...
def _insert_bank_list(connection: engine.Connection, meta: MetaData, ratios: dict, b_date: date):
    """
    Аргументы: соединение с бд, метаданные бд, дата и список наименований банков.
...
def _mart_bank_list(connection: engine.Connection, meta: MetaData,
    ...
    ratios = get_ratios.get_bank_list_ratios(connection, meta, list(set(names).difference(parsed_query)),
                                                                                b_date)
    return ratios

def form_bank_list_mart(db_engine: engine, connection: engine.Connection, meta: MetaData,
