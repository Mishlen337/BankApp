...
def get_bank_ratios(connection: engine.Connection, meta: MetaData,
                    name: str, b_dates: list) -> dict:
    """
    Аргументы: соединение с бд, метаданные бд, имя банка и промежуток дат
    в виде списка на начало месяца.
    ...
    for b_date in b_dates:
        bank_ratios[b_date] = reliability_stability.get_reliability_stability(
            connection, meta, name, b_date) + \
...
def get_bank_list_ratios(connection: engine.Connection, meta: MetaData,
    ...
    for name in names:
        bank_list_ratios[name] = reliability_stability.\
            get_reliability_stability(connection, meta, name, b_date) +\
            efficiency.get_efficiency(connection, meta, name, b_date)
