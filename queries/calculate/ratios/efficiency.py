...
def _get_net_income_month(connection: engine.Connection, meta: MetaData,
    ...
    if b_date >= date(b_date.year, 1, 1) and b_date < date(b_date.year, 4, 1):
        net_income = _get_net_income_quater(
            connection, meta, name, date(
                b_date.year, 1, 1)) / 4
...
def _get_capital(connection: engine.Connection, meta: MetaData, name: str,
                 b_date: date) -> float:
    """
...
def _get_net_assets(connection: engine.Connection, meta: MetaData,
    ...
    capital_adequacy = reliability_stability._get_capital_adequacy(
...
def get_efficiency(
    ...
    """
    capital_profitability = _get_capital_profitability(
        connection, meta, name, b_date)
